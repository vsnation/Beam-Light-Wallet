...
# This script is for WSL (Windows Subsystem for Linux) environments
#
...
# Platform: WSL runs the Linux ELF binaries, so this reads the manifest's
# "linux" entry. The manifest's "windows" entry (assets prefixed "win-", not
# "windows-") is for the native installers under build/windows/.
PLATFORM="linux"
